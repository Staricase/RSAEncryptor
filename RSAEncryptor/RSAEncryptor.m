...
- (NSData*) rsaDecryptData:(NSData*)data {
    ...
    size_t cipherBufferSize = SecKeyGetBlockSize(key);
    uint8_t *cipherBuffer = malloc(cipherBufferSize * sizeof(uint8_t));
    size_t block_size = cipherBufferSize;
    
    size_t blockCount = (size_t)ceil([data length] / (double)block_size);
    NSMutableData *decryptedData = [[NSMutableData alloc] init] ;
    for (int i=0; i<blockCount; i++) {
        NSInteger bufferSize = MIN(cipherBufferSize,[data length] - i * block_size);
        NSData *buffer = [data subdataWithRange:NSMakeRange(i * block_size, bufferSize)];
        OSStatus status = SecKeyDecrypt(key, kSecPaddingPKCS1, (const uint8_t *)[buffer bytes], [buffer length], cipherBuffer, &cipherBufferSize);
        if (status == noErr){
            NSData *encryptedBytes = [[NSData alloc] initWithBytes:(const void *)cipherBuffer length:cipherBufferSize];
            [decryptedData appendData:encryptedBytes];
            cipherBufferSize = block_size;
        }else{
            if (decryptedData) {
                free(cipherBuffer);
            }
...
+ (SecKeyRef)addPrivateKey:(NSString *)key{
    ...
    // This will be base64 encoded, decode it.
    NSData *data = base64_decode(key);
    data = [RSAEncryptor stripPrivateKeyHeader:data];
    if(!data){
        return nil;
    }
...
+ (NSString *)encryptString:(NSString *)str privateKey:(NSString *)privKey{
    NSData *data = [RSAEncryptor encryptData:[str dataUsingEncoding:NSUTF8StringEncoding] privateKey:privKey];
...
+ (NSData *)encryptData:(NSData *)data privateKey:(NSString *)privKey{
    ...
    SecKeyRef keyRef = [RSAEncryptor addPrivateKey:privKey];
    if(!keyRef){
        return nil;
    }
...
+ (NSData *)decryptData:(NSData *)data privateKey:(NSString *)privKey{
    ...
    return [RSAEncryptor decryptData:data withKeyRef:keyRef];
}
...
+ (NSData *)encryptData:(NSData *)data publicKey:(NSString *)pubKey{
    ...
    SecKeyRef keyRef = [RSAEncryptor addPublicKey:pubKey];
    if(!keyRef){
        return nil;
    }
    return [RSAEncryptor encryptData:data withKeyRef:keyRef];
...
+ (NSString *)decryptString:(NSString *)str publicKey:(NSString *)pubKey{
    NSData *data = [[NSData alloc] initWithBase64EncodedString:str options:NSDataBase64DecodingIgnoreUnknownCharacters];
    data = [RSAEncryptor decryptData:data publicKey:pubKey];
    NSString *ret = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    return ret;
}
